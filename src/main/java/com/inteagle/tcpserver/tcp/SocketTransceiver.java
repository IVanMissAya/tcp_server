package com.inteagle.tcpserver.tcp;
...
public abstract class SocketTransceiver implements Runnable {
        ...
        while (runFlag) {
            try {
                ...
                while (in.read(bytes) != -1) {
                    ret = ret + ByteHexUtil.bytesToHexString(bytes) + " ";
                    //一个请求
                    if (in.available() == 0) {
                        this.onReceive(addr, ret);
                        ret = "";
                    }
                }
     ...
     * @param data 收到的字符串
     */
    public abstract void onReceive(InetAddress addr, String data);
